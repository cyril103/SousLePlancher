...
const KINDS := ["food", "food", "wood", "wood", "fiber", "fiber", "wood"]
...
static func capture(game: Node) -> Dictionary:
	...
	return {
		...
		"stock": game.stock.duplicate(), "patches": patches, "buildings": buildings,
...
static func validate(data: Variant) -> String:
	...
	if data.format != "SousLePlancher/checkpoint" or not number(data.version, VERSION, VERSION, true): return "Version de sauvegarde incompatible."
	...
	if not data.patches is Array or data.patches.size() != KINDS.size(): return "Carte de ressources incompatible."
	...
	for building in data.buildings:
		...
		if not building.kind in ["shelter", "workshop"]: return "Type de bâtiment inconnu."
	...
	for assignment in data.assignments:
		if not number(assignment, -1, 6, true): return "Affectation invalide."
